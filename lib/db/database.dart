import 'package:authentication/db/auth_entries.dart';
...
class AppDatabase extends _$AppDatabase {
  ...
  /// 返回新插入行的自增 ID。
  Future<int> insertAuthEntry(AuthEntriesCompanion entry) =>
      into(authEntries).insert(entry);

  /// 更新现有的身份验证条目。
  ///
  /// [entry] 必须包含有效的 ID 字段以定位数据库中的行。
  /// 返回一个布尔值，表示更新是否成功。
  Future<bool> updateAuthEntry(AuthEntry entry) =>
      update(authEntries).replace(entry);

  /// 根据 ID 删除指定的身份验证条目。
  ///
  /// [id] 要删除条目的主键。
  /// 返回受影响的行数。
  Future<int> deleteAuthEntry(int id) =>
      (delete(authEntries)..where((t) => t.id.equals(id))).go();
  ...
  /// [id] 要监听条目的主键。
  /// 如果找不到该 ID，流将推送 null。
  Stream<AuthEntry?> watchAuthEntry(int id) =>
      (select(authEntries)..where((t) => t.id.equals(id))).watchSingleOrNull();
  ...
  Future<AuthEntry?> getAuthEntryByAccountAndIssuer(
    String account,
    String issuer,
  ) =>
      (select(authEntries)
            ..where((t) => t.account.equals(account) & t.issuer.equals(issuer)))
          .getSingleOrNull();

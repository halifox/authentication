...
/// 返回值为标题的 [Widget] 组件。
Widget TopBarTitle(BuildContext context, String data) {
  return Text(data, style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Theme.of(context).colorScheme.onSurface));
}
...
Widget TopBarIconButton(BuildContext context, IconData? icon, VoidCallback? onPressed) {
  return Visibility(
    ...
    child: ElevatedButton(
      ...
      child: Icon(icon, size: _TopBarIconSize, color: Theme.of(context).colorScheme.onPrimaryContainer),
    ),
  );
}
...
Widget TopBarIconButtonPlaceholder() {
  return SizedBox(width: _TopBarIconButtonSize, height: _TopBarIconButtonSize);
}
